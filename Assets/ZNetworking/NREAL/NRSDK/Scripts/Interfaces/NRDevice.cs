...
namespace NRKernal
{
    ...
    public partial class NRDevice : SingleTon<NRDevice>
    {
        ...
        private static void OnGlassesDisconnectEvent(ulong glasses_control_handle, ulong user_data, GlassesDisconnectReason reason)
        {
            Debug.Log("[NRDevice] OnGlassesDisconnectEvent:" + reason.ToString());
            switch (reason)
            {
                case GlassesDisconnectReason.GLASSES_DEVICE_DISCONNECT:
                    OnGlassesPlugOut(glasses_control_handle, user_data);
                    break;
