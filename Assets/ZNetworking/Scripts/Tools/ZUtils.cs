...
public static class ZUtils
{
    ...
    public static void Look(bool fade, Transform watcher, Vector3 target, float rate = 1, float speed = 200)
    {
        Vector3 directionToFace = target - watcher.position;//sphere 就是要注视的obj
        ...
        else
        {
            ...
            watcher.rotation = Quaternion.Slerp(watcher.rotation, targetRotation, Time.deltaTime * speed / rate);//在1秒内注视LookAt完毕
        }

        watcher.eulerAngles = new Vector3(0, watcher.eulerAngles.y, 0);
    }
